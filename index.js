...
async function initDatabase() {
  ...
  await pool.query(`ALTER TABLE madden_ea_connections ADD COLUMN IF NOT EXISTS token_type TEXT`);
  await pool.query(`ALTER TABLE madden_ea_connections ADD COLUMN IF NOT EXISTS token_scope TEXT`);
  await pool.query(`ALTER TABLE madden_ea_connections ADD COLUMN IF NOT EXISTS exchange_status TEXT`);
  await pool.query(`ALTER TABLE madden_ea_connections ADD COLUMN IF NOT EXISTS raw_token_payload JSONB`);
  await pool.query(`ALTER TABLE madden_ea_connections ADD COLUMN IF NOT EXISTS raw_personas_payload JSONB`);
  await pool.query(`ALTER TABLE madden_ea_connections ADD COLUMN IF NOT EXISTS raw_franchises_payload JSONB`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS madden_ea_personas (
      id UUID PRIMARY KEY,
      guild_id TEXT NOT NULL,
      league_id UUID NOT NULL REFERENCES leagues(league_id) ON DELETE CASCADE,
      user_id TEXT NOT NULL,
      ...
      UNIQUE (league_id, user_id, persona_id)
    )
  `);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS madden_ea_franchises (
      id UUID PRIMARY KEY,
      guild_id TEXT NOT NULL,
      league_id UUID NOT NULL REFERENCES leagues(league_id) ON DELETE CASCADE,
      user_id TEXT NOT NULL,
      ...
      raw_payload JSONB,
      imported_at TIMESTAMP NOT NULL DEFAULT NOW(),
      UNIQUE (league_id, user_id, franchise_id)
    )
  `);
  ...
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('madden_ea_code_modal:')) {
      ...
      try {
        ...
        const components = buildMaddenEaPersonaComponentsFromRows(leagueId, personas);
        ...
        await pool.query(
          `UPDATE madden_ea_connections SET exchange_status = 'failed', last_error = $3, updated_at = NOW() WHERE league_id = $1 AND user_id = $2 AND provider = 'ea_direct'`,
          [leagueId, interaction.user.id, error.message || 'Token exchange failed']
        ).catch(() => null);
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('madden_ea_persona_select:')) {
      ...
      let personaName = personaId === 'default_persona' ? 'Default EA Persona' : personaId;
      const personaRows = await getEaPersonasForConnection(leagueId, interaction.user.id).catch(() => []);
      const personaRow = personaRows.find(row => String(row.persona_id) === String(personaId));
      if (personaRow?.persona_name) personaName = personaRow.persona_name;
      ...
      if (EA_DIRECT_SELECT_LEAGUE_URL && connection?.access_token_encrypted) {
        ...
        try {
          ...
          const franchiseUrl = EA_DIRECT_FRANCHISES_URL
            ...
            .replaceAll('{LEAGUE_ID}', encodeURIComponent(leagueId));
          const payload = await fetchEaJson(franchiseUrl, accessToken, 'EA franchises');
          await saveEaFranchises(interaction.guild?.id || league.guild_id, leagueId, interaction.user.id, personaId, payload);
          await pool.query(
            `UPDATE madden_ea_connections SET raw_franchises_payload = $3, exchange_status = 'franchises_fetched', updated_at = NOW()
             WHERE league_id = $1 AND user_id = $2 AND provider = 'ea_direct'`,
            [leagueId, interaction.user.id, JSON.stringify(payload)]
          );
          connection = await getMaddenEaConnection(leagueId, interaction.user.id);
        } catch (error) {
          await pool.query(
            `UPDATE madden_ea_connections SET last_error = $3, updated_at = NOW() WHERE league_id = $1 AND user_id = $2 AND provider = 'ea_direct'`,
            [leagueId, interaction.user.id, error.message || 'Franchise fetch failed']
          ).catch(() => null);
        }
      }
      ...
      await interaction.update({
        content: leagueChoices.length
          ...
          : franchises.length
            ...
            : 'Persona token created. Blaze getLeagues adapter is pending, so use the placeholder franchise for now.',
        embeds: [buildMaddenEaConnectionEmbed(league, connection)],
...
if (shopSubcommand === 'view') {
    ...
    if (interaction.commandName === 'league') {
      ...
      if (leagueSubcommand === 'settings') {
        ...
        if (selectedLeagueRole || clearLeagueRole) {
          if (!(await userCanUseLeagueSetup(interaction, activeLeague))) {
            await interaction.reply({ content: 'You do not have permission to update league settings.', ephemeral: true });
            return;
          }
          ...
          await pool.query(
            ...
            [activeLeague.league_id, clearLeagueRole ? null : selectedLeagueRole.id]
          );
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Sportsbook Channel', value: activeLeague.sportsbook_channel_id ? '<#' + activeLeague.sportsbook_channel_id + '>' : 'Not set', inline: true },
            { name: 'Sportsbook Feed', value: activeLeague.sportsbook_feed_enabled === false ? 'Disabled' : 'Enabled', inline: true },
            { name: 'Playoff Teams', value: String(activeLeague.playoff_team_count || 'Default'), inline: true }
          )
          .setFooter({ text: 'GG Sports • League Settings' })
          .setTimestamp();

        const updateMessage = selectedLeagueRole
          ? 'League role updated. '
          : clearLeagueRole
            ? 'League role cleared. '
            : '';

        await interaction.reply({ content: updateMessage || null, embeds: [embed], ephemeral: true });
        return;
      }
      ...
      if (leagueSubcommand === 'sportsbookchannel') {
        ...
        await pool.query(
          `INSERT INTO league_settings (league_id, sportsbook_channel_id, updated_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT (league_id)
           DO UPDATE SET sportsbook_channel_id = $2, updated_at = NOW()`,
      ...
      if (leagueSubcommand === 'sportsbookfeed') {
        if (!(await userCanUseLeagueSetup(interaction, league))) {
          await interaction.reply({ content: 'You do not have permission to update sportsbook settings.', ephemeral: true });
          return;
        }
        ...
        await pool.query(
          `INSERT INTO league_settings (league_id, sportsbook_feed_enabled, updated_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT (league_id)
           DO UPDATE SET sportsbook_feed_enabled = $2, updated_at = NOW()`,
          [activeLeague.league_id, enabled]
        );

        await interaction.reply({ content: 'Sportsbook live feed for **' + activeLeague.league_name + '** is now **' + (enabled ? 'enabled' : 'disabled') + '**.', ephemeral: true });
        return;
      }

      if (leagueSubcommand === 'sportsbooksettings') {
        const leagueName = interaction.options.getString('league');
        const activeLeague = leagueName ? await getLeagueByName(interaction.guild.id, leagueName) : await getDefaultLeague(interaction.guild.id);
        ...
        const embed = new EmbedBuilder()
          .setTitle('Sportsbook Settings • ' + activeLeague.league_name)
          .setColor(0x57F287)
          .addFields(
            { name: 'Feed Enabled', value: activeLeague.sportsbook_feed_enabled === false ? 'No' : 'Yes', inline: true },
            { name: 'Feed Channel', value: activeLeague.sportsbook_channel_id ? '<#' + activeLeague.sportsbook_channel_id + '>' : 'Not set', inline: true },
            { name: 'Big Bet Threshold', value: String(activeLeague.sportsbook_big_bet_threshold || 1000), inline: true },
            { name: 'Monster Parlay Legs', value: String(activeLeague.sportsbook_monster_parlay_legs || 4), inline: true }
          )
...
async function grantVisualAvatarItem(guildId, userId, itemName, slot, source = 'unlock') {
  await pool.query(
    `INSERT INTO user_avatar_inventory (guild_id, user_id, item_name, slot, source)
     ...
     DO UPDATE SET slot = $4, source = COALESCE(user_avatar_inventory.source, $5)`,
    [guildId, userId, itemName, slot, source]
  );
}
...
const PROFILE_BADGE_DEFINITIONS = [
  { key: 'champion', icon: '🏆', label: 'Champion', description: 'Won at least one championship.' },
  { key: 'dynasty', icon: '👑', label: 'Dynasty', description: 'Won 5+ championships.' },
  { key: 'legacy_goat', icon: '🐐', label: 'GOAT Legacy', description: 'Reached GOAT legacy tier.' },
  { key: 'legacy_legend', icon: '🔥', label: 'Legend Legacy', description: 'Reached Legend legacy tier.' },
  { key: 'legacy_elite', icon: '💎', label: 'Elite Legacy', description: 'Reached Elite legacy tier.' },
  { key: 'activity_grinder', icon: '💀', label: 'Grinder Activity', description: 'Reached Grinder/Icon activity tier.' },
  { key: 'activity_dedicated', icon: '🏹', label: 'Dedicated Activity', description: 'Reached Dedicated/Elite activity tier.' },
  { key: 'sportsbook_winner', icon: '🎰', label: 'Sportsbook Winner', description: 'Won at least one sportsbook bet.' },
  { key: 'sharp_bettor', icon: '🎯', label: 'Sharp Bettor', description: 'Won 10+ sportsbook bets.' },
  { key: 'high_roller', icon: '💰', label: 'High Roller', description: 'Reached 10,000+ sportsbook profit.' },
  { key: 'collector', icon: '🛒', label: 'Collector', description: 'Purchased 5+ shop/inventory items.' },
...
function badgeDefinition(key) {
  return PROFILE_BADGE_DEFINITIONS.find(badge => badge.key === key) || null;
}

async function awardProfileBadge(guildId, userId, key, source = null) {
  const badge = badgeDefinition(key);
  if (!badge) return;

  await pool.query(
    `INSERT INTO user_badges (guild_id, user_id, badge_key, badge_label, badge_icon, source, description)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     ON CONFLICT (guild_id, user_id, badge_key)
     DO UPDATE SET badge_label = $4, badge_icon = $5, source = COALESCE($6, user_badges.source), description = $7`,
    [guildId, userId, badge.key, badge.label, badge.icon, source, badge.description]
  );
}

async function syncExpandedProfileBadges(guildId, userId, recognition = null) {
  const row = recognition || (await pool.query(
    `SELECT * FROM user_recognition WHERE guild_id = $1 AND user_id = $2 LIMIT 1`,
    [guildId, userId]
  )).rows[0] || {};
  ...
  const sportsbookWins = Number(row.sportsbook_wins || 0);
  const sportsbookProfit = Number(row.sportsbook_profit || 0);
  const ticketsResolved = Number(row.tickets_resolved || 0);
  ...
  if (championships >= 1) await awardProfileBadge(guildId, userId, 'champion', 'championship');
  if (championships >= 5) await awardProfileBadge(guildId, userId, 'dynasty', 'championship');
  if (legacyName === 'GOAT') await awardProfileBadge(guildId, userId, 'legacy_goat', 'legacy');
  if (legacyName === 'Legend') await awardProfileBadge(guildId, userId, 'legacy_legend', 'legacy');
  if (legacyName === 'Elite') await awardProfileBadge(guildId, userId, 'legacy_elite', 'legacy');
  if (activityName === 'Grinder') await awardProfileBadge(guildId, userId, 'activity_grinder', 'activity');
  if (activityName === 'Dedicated') await awardProfileBadge(guildId, userId, 'activity_dedicated', 'activity');
  if (sportsbookWins >= 1) await awardProfileBadge(guildId, userId, 'sportsbook_winner', 'sportsbook');
  if (sportsbookWins >= 10) await awardProfileBadge(guildId, userId, 'sharp_bettor', 'sportsbook');
  if (sportsbookProfit >= 10000) await awardProfileBadge(guildId, userId, 'high_roller', 'sportsbook');
  if (ticketsResolved >= 10) await awardProfileBadge(guildId, userId, 'ticket_helper', 'tickets');
...
async function getExpandedUserBadges(guildId, userId, recognition = null) {
  ...
  const result = await pool.query(
    `SELECT * FROM user_badges WHERE guild_id = $1 AND user_id = $2 ORDER BY unlocked_at ASC LIMIT 20`,
    [guildId, userId]
  );
  return result.rows;
}
...
function formatBadgeLine(badge) {
  return badge.badge_icon + ' **' + badge.badge_label + '**' + (badge.description ? ' — ' + badge.description : '');
}

function buildBadgesEmbed(user, badges) {
  const NL = String.fromCharCode(10);
  return new EmbedBuilder()
    .setTitle(user.username + ' Badges')
    .setColor(0xFEE75C)
    .setThumbnail(user.displayAvatarURL({ dynamic: true }))
    .setDescription(badges.length ? badges.map(formatBadgeLine).join(NL).slice(0, 4096) : 'No badges unlocked yet.')
    .setFooter({ text: 'GG Sports • Badges' })
    .setTimestamp();
}

function buildBadgeProgressEmbed(user, recognition = {}) {
  const championships = Number(recognition.championships || 0);
  const sportsbookWins = Number(recognition.sportsbook_wins || 0);
  const sportsbookProfit = Number(recognition.sportsbook_profit || 0);
  const ticketsResolved = Number(recognition.tickets_resolved || 0);
  const gamesPlayed = Number(recognition.games_played || 0);

  const lines = [
    '🏆 Champion: ' + Math.min(championships, 1) + '/1 championship',
    '👑 Dynasty: ' + Math.min(championships, 5) + '/5 championships',
    '🎰 Sportsbook Winner: ' + Math.min(sportsbookWins, 1) + '/1 win',
    '🎯 Sharp Bettor: ' + Math.min(sportsbookWins, 10) + '/10 wins',
    '💰 High Roller: ' + Math.min(sportsbookProfit, 10000) + '/10000 profit',
    '🛠️ Ticket Helper: ' + Math.min(ticketsResolved, 10) + '/10 tickets',
    '🎮 League Grinder: ' + Math.min(gamesPlayed, 25) + '/25 games',
  ];

  return new EmbedBuilder()
    .setTitle(user.username + ' Badge Progress')
    .setColor(0x5865F2)
...
function getEaEncryptionKey() {
  const source = EA_DIRECT_ENCRYPTION_KEY || process.env.DATABASE_URL || 'gg-sports-dev-key';
  return crypto.createHash('sha256').update(source).digest();
}

function encryptEaSecret(value) {
  if (!value) return null;
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', getEaEncryptionKey(), iv);
  const encrypted = Buffer.concat([cipher.update(String(value), 'utf8'), cipher.final()]);
  const tag = cipher.getAuthTag();
  return Buffer.concat([iv, tag, encrypted]).toString('base64');
}

function decryptEaSecret(value) {
  if (!value) return null;
  const buffer = Buffer.from(value, 'base64');
  const iv = buffer.subarray(0, 12);
  const tag = buffer.subarray(12, 28);
  const encrypted = buffer.subarray(28);
  const decipher = crypto.createDecipheriv('aes-256-gcm', getEaEncryptionKey(), iv);
...
async function fetchEaJson(url, accessToken, label) {
  ...
  if (!response.ok) {
    throw new Error(label + ' request failed: HTTP ' + response.status + ' • ' + (payload.error_description || payload.error || text).slice(0, 300));
  }

  return payload;
}

function extractEaPersonaRows(payload) {
  if (Array.isArray(payload)) return payload;
  if (!payload || typeof payload !== 'object') return [];
  for (const key of ['personas', 'persona', 'data', 'users']) {
    if (Array.isArray(payload[key])) return payload[key];
  }
  if (payload.data && typeof payload.data === 'object') {
    for (const key of ['personas', 'persona', 'users']) {
      if (Array.isArray(payload.data[key])) return payload.data[key];
    }
  }
  return [];
...
function extractEaFranchiseRows(payload) {
  if (Array.isArray(payload)) return payload;
  if (!payload || typeof payload !== 'object') return [];
  for (const key of ['franchises', 'leagues', 'clubs', 'data']) {
    if (Array.isArray(payload[key])) return payload[key];
  }
  if (payload.data && typeof payload.data === 'object') {
    for (const key of ['franchises', 'leagues', 'clubs']) {
      if (Array.isArray(payload.data[key])) return payload.data[key];
    }
  }
  return [];
...
async function saveEaPersonas(guildId, leagueId, userId, payload) {
  const rows = extractEaPersonaRows(payload);
  let imported = 0;

  for (const row of rows) {
    const personaId = String(getFirstValue(row, ['personaId', 'persona_id', 'id', 'pid'], '') || '').trim();
    if (!personaId) continue;

    const personaName = getFirstValue(row, ['personaName', 'persona_name', 'displayName', 'name', 'gamertag'], personaId);
    const platform = getFirstValue(row, ['platform', 'namespaceName', 'console'], null);
...
async function saveEaFranchises(guildId, leagueId, userId, personaId, payload) {
  const rows = extractEaFranchiseRows(payload);
  let imported = 0;

  for (const row of rows) {
    const franchiseId = String(getFirstValue(row, ['franchiseId', 'franchise_id', 'leagueId', 'league_id', 'id'], '') || '').trim();
    if (!franchiseId) continue;

    const franchiseName = getFirstValue(row, ['franchiseName', 'franchise_name', 'leagueName', 'name'], franchiseId);
    const platform = getFirstValue(row, ['platform', 'namespaceName', 'console'], null);
...
function extractEaStandingsRequestContextFromHub(hubPayload, leagueId) {
  ...
  // IMPORTANT 7J-5AX:
  // Do NOT blindly use availableWeekInfoList[0].
  // EA keeps preseason weeks in that list after the franchise advances.
  // Prefer true current/next hub fields first.
  const hubDisplayWeek =
    parseNumberOrNull(getAnyValue(hubPayload, ['displayWeek', 'displayedWeek'], null)) ??
    parseNumberOrNull(getAnyValue(seasonInfo, ['displayWeek', 'displayedWeek'], null));

  const hubWeekIndex =
    parseNumberOrNull(getAnyValue(hubPayload, ['weekIndex'], null)) ??
    parseNumberOrNull(getAnyValue(seasonInfo, ['weekIndex'], null));

  const nextSeasonWeek =
    parseNumberOrNull(getAnyValue(scheduleInfo, ['nextSeasonWeek'], null));

  const nextSeasonWeekType =
    parseNumberOrNull(getAnyValue(scheduleInfo, ['nextSeasonWeekType'], null));

  const stageIndex =
    parseNumberOrNull(getAnyValue(hubPayload, ['stageIndex'], null)) ??
    parseNumberOrNull(getAnyValue(seasonInfo, ['stageIndex'], null));
  ...
  if (nextSeasonWeek !== null && nextSeasonWeekType !== null) {
    currentWeekInfo = availableWeeks.find(w =>
      parseNumberOrNull(getAnyValue(w, ['seasonWeek'], null)) === nextSeasonWeek &&
      parseNumberOrNull(getAnyValue(w, ['seasonWeekType'], null)) === nextSeasonWeekType
    ) || null;
  }

  if (!currentWeekInfo && hubDisplayWeek !== null) {
    currentWeekInfo = availableWeeks.find(w =>
      parseNumberOrNull(getAnyValue(w, ['weekIndex', 'displayWeek'], null)) === hubDisplayWeek
    ) || null;
  }

  if (!currentWeekInfo && hubWeekIndex !== null) {
    currentWeekInfo = availableWeeks.find(w =>
      parseNumberOrNull(getAnyValue(w, ['weekIndex'], null)) === hubWeekIndex
    ) || null;
  }

  // Last fallback only: first matching inferred week type, never hard-code preseason.
  if (!currentWeekInfo && inferredWeekType !== null) {
    currentWeekInfo = availableWeeks.find(w =>
      parseNumberOrNull(getAnyValue(w, ['seasonWeekType', 'weekType'], null)) === inferredWeekType
    ) || null;
  }
  ...
  const displayedWeek =
    ...
    (weekType === 0
      ? 'Preseason Week ' + String((seasonWeek ?? 0) + 1)
      : 'Week ' + String((seasonWeek ?? 0) + 1));
  ...
  const gamesPlayedCount = parseNumberOrNull(getAnyValue(hubPayload, ['gamesPlayedCount'], null));
  const gameTotalCount = parseNumberOrNull(getAnyValue(hubPayload, ['gameTotalCount'], null));
  ...
  const ctx = {
    ...
    // Current week fields, now based on true hub state.
    displayWeek: hubDisplayWeek,
    weekIndex,
    seasonWeek,
    week: seasonWeek,
    weekType,
...
function getEaHubSeasonModeLabel(hubPayload, leagueId = null) {
  ...
  if (ctx.isRegularSeason || ctx.seasonWeekType === 1 || ctx.weekType === 1) {
    return ctx.displayedWeek || ('Regular Season Week ' + String((ctx.seasonWeek ?? ctx.weekIndex ?? 0) + 1));
  }
...
function parseMaddenScoreValue(value) {
...
function resolveMaddenScoresFromRawGame(gameRow) {
  ...
  const homeCandidates = [
    gameRow?.home_score,
    gameRow?.homeScore,
    getAnyValue(rawGame, ['homeScore', 'home_score', 'homeTeamScore', 'homePts', 'homePoints', 'homeFinalScore', 'homeTeamFinalScore'], null),
    getAnyValue(rawGame, ['homeStatScore', 'homeScoreValue', 'homeTotalScore'], null),
  ];

  const awayCandidates = [
    gameRow?.away_score,
    gameRow?.awayScore,
    getAnyValue(rawGame, ['awayScore', 'away_score', 'awayTeamScore', 'awayPts', 'awayPoints', 'awayFinalScore', 'awayTeamFinalScore'], null),
    getAnyValue(rawGame, ['awayStatScore', 'awayScoreValue', 'awayTotalScore'], null),
  ];
  ...
  for (const candidate of homeCandidates) {
    const parsed = parseMaddenScoreValue(candidate);
    if (parsed !== null) {
      homeScore = parsed;
      break;
    }
  ...
  // If EA only stores a text result like "24-17", parse it cautiously.
  if ((homeScore === null || awayScore === null)) {
    const resultText = String(getAnyValue(rawGame, ['score', 'finalScore', 'resultText', 'gameResultText'], '') || '');
    const match = resultText.match(/(\d+)\s*[-–]\s*(\d+)/);
    if (match) {
      // Unknown orientation, only use this when both explicit scores are missing.
      if (awayScore === null) awayScore = Number(match[1]);
      if (homeScore === null) homeScore = Number(match[2]);
    }
  }

  const hasRealScore = homeScore !== null && awayScore !== null && (homeScore !== 0 || awayScore !== 0);
  ...
  return {
    ...
    hasRealScore,
    rawScoreKeys: {
      homeScore: getAnyValue(rawGame, ['homeScore'], null),
      awayScore: getAnyValue(rawGame, ['awayScore'], null),
      homeTeamScore: getAnyValue(rawGame, ['homeTeamScore'], null),
      awayTeamScore: getAnyValue(rawGame, ['awayTeamScore'], null),
      homePts: getAnyValue(rawGame, ['homePts', 'homePoints'], null),
      awayPts: getAnyValue(rawGame, ['awayPts', 'awayPoints'], null),
      score: getAnyValue(rawGame, ['score', 'finalScore', 'resultText', 'gameResultText'], null),
    },
  };
}
...
async function backfillMaddenGameScoresFromRawPayload(guild, league) {
  const gamesResult = await pool.query(
    `SELECT *
     FROM madden_imported_games
     WHERE guild_id = $1 AND league_id = $2`,
...
function hasUsableMaddenScorePair(homeScore, awayScore) {
  const h = parseNumberOrNull(homeScore);
  const a = parseNumberOrNull(awayScore);
  if (h === null || a === null) return false;
  return h > 0 || a > 0;
}


async function loadImportedMaddenTeamSeedMap(guild, league) {
  const rows = await pool.query(
    `SELECT team_name, wins, losses, ties, points_for, points_against, raw_payload
  ...
  for (const row of rows.rows || []) {
    const canonicalName = canonicalMaddenTeamNameFromAny(row.team_name);
    if (!canonicalName) continue;

    map.set(canonicalName.toLowerCase(), {
      canonicalName,
      teamName: canonicalName,
      displayName: canonicalName,
      wins: parseNumberOrNull(row.wins) ?? 0,
      losses: parseNumberOrNull(row.losses) ?? 0,
      ties: parseNumberOrNull(row.ties) ?? 0,
      teamTotalPointsScored: parseNumberOrNull(row.points_for) ?? 0,
      teamTotalPointsAllowed: parseNumberOrNull(row.points_against) ?? 0,
      sourcePaths: ['db:madden_imported_team_stats'],
      seededFromDb: true,
    });
  }
...
function mergeTeamIntoDiscoveryMap(map, team, sourcePath = 'unknown') {
  ...
  const canonicalName = canonicalMaddenTeamNameFromAny(
    team.canonicalName || team.displayName || team.teamName || team.name || team.shortName || team.longName
  );
  ...
  const existing = map.get(key) || {
    ...
    losses: 0,
    ties: 0,
    teamTotalPointsScored: 0,
    teamTotalPointsAllowed: 0,
    sourcePaths: [],
  };

  const merged = { ...existing, canonicalName };

  function preferNumber(field, value) {
    const parsed = parseNumberOrNull(value);
    if (parsed === null) return;

    const current = parseNumberOrNull(merged[field]);
    // For PF/PA, never replace nonzero with zero.
    if (
      ['teamTotalPointsScored', 'teamTotalPointsAllowed'].includes(field) &&
      parsed === 0 &&
      current !== null &&
    ...
    // For record fields, use latest numeric value.
    merged[field] = parsed;
  }

  preferNumber('wins', team.wins);
  preferNumber('losses', team.losses);
  preferNumber('ties', team.ties);

  preferNumber('teamTotalPointsScored',
    team.teamTotalPointsScored ?? team.points_for ?? team.pointsFor ?? team.pf
  );
  preferNumber('teamTotalPointsAllowed',
    team.teamTotalPointsAllowed ?? team.points_against ?? team.pointsAgainst ?? team.pa
  );

  preferNumber('teamTotalPointsScoredRank', team.teamTotalPointsScoredRank ?? team.pfRank);
  preferNumber('teamTotalPointsAllowedRank', team.teamTotalPointsAllowedRank ?? team.paRank);

  for (const field of [
    'ovr', 'offenseOvr', 'defenseOvr',
    'totalPassYards', 'totalRushYards', 'totalDefPassYards', 'totalDefRushYards',
    'passTds', 'rushTds', 'sacks', 'takeaways', 'giveAways',
    'thirdDownPercent', 'redZonePercent',
  ]) {
    preferNumber(field, team[field]);
  }

  for (const field of ['displayName', 'teamName', 'logoId', 'userName', 'isHuman']) {
    if (team[field] !== null && team[field] !== undefined && team[field] !== '') merged[field] = team[field];
  }

  if (Array.isArray(team.topThreats) && team.topThreats.length) {
    if (!Array.isArray(merged.topThreats) || !merged.topThreats.length) merged.topThreats = team.topThreats;
  }

  merged.sourcePaths = Array.from(new Set([...(existing.sourcePaths || []), sourcePath].filter(Boolean))).slice(0, 20);
  map.set(key, merged);
}


function looksLikeFullLeagueTeamStatRow(obj) {
  if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return false;
  const keys = Object.keys(obj);
  ...
  const hasName =
    lower.has('team') ||
    lower.has('teamname') ||
    lower.has('displayname') ||
    lower.has('shortname') ||
    lower.has('longname') ||
    lower.has('teamassetname') ||
    lower.has('team_assetname') ||
    lower.has('name');
  ...
  const hasPfPa =
    ...
    lower.has('pa') ||
    lower.has('pointsfor') ||
    lower.has('pointsagainst') ||
    lower.has('points_for') ||
    lower.has('points_against') ||
    lower.has('teamtotalpointsscored') ||
    lower.has('teamtotalpointsallowed') ||
    lower.has('totalpointsscored') ||
    lower.has('totalpointsallowed');
  ...
  const hasRanks =
    lower.has('teamtotalpointsscoredrank') ||
    lower.has('teamtotalpointsallowedrank') ||
    lower.has('pfrank') ||
    lower.has('parank') ||
    lower.has('offenserank') ||
    lower.has('defenserank');

  return hasName && (hasPfPa || hasRecord || hasRanks);
}

function normalizeFullLeagueAnalyticsRow(row, sourcePath = '') {
  if (!row || typeof row !== 'object') return null;

  const teamName = canonicalMaddenTeamNameFromAny(
    getAnyValue(row, [
      'canonicalName',
      'displayName',
      'teamName',
  ...
  return {
    ...
    displayName: getAnyValue(row, ['displayName', 'teamName', 'team', 'shortName', 'longName', 'name'], teamName),
    teamName,
    wins: parseNumberOrNull(getAnyValue(row, ['wins', 'win', 'w'], null)),
    losses: parseNumberOrNull(getAnyValue(row, ['losses', 'loss', 'l'], null)),
    ties: parseNumberOrNull(getAnyValue(row, ['ties', 'tie', 't'], null)),
    teamTotalPointsScored: parseNumberOrNull(getAnyValue(row, [
      'teamTotalPointsScored',
      'totalPointsScored',
      'pointsFor',
      'points_for',
      'pf',
    ...
    teamTotalPointsAllowed: parseNumberOrNull(getAnyValue(row, [
      'teamTotalPointsAllowed',
      'totalPointsAllowed',
      'pointsAgainst',
      'points_against',
      'pa',
    ...
    teamTotalPointsScoredRank: parseNumberOrNull(getAnyValue(row, ['teamTotalPointsScoredRank', 'pfRank', 'pointsForRank'], null)),
    teamTotalPointsAllowedRank: parseNumberOrNull(getAnyValue(row, ['teamTotalPointsAllowedRank', 'paRank', 'pointsAgainstRank'], null)),
    totalPassYards: parseNumberOrNull(getAnyValue(row, ['totalPassYards', 'passYards'], null)),
    totalRushYards: parseNumberOrNull(getAnyValue(row, ['totalRushYards', 'rushYards'], null)),
    totalDefPassYards: parseNumberOrNull(getAnyValue(row, ['totalDefPassYards', 'defPassYards'], null)),
    totalDefRushYards: parseNumberOrNull(getAnyValue(row, ['totalDefRushYards', 'defRushYards'], null)),
    passTds: parseNumberOrNull(getAnyValue(row, ['passTds', 'passingTds'], null)),
    rushTds: parseNumberOrNull(getAnyValue(row, ['rushTds', 'rushingTds'], null)),
    sacks: parseNumberOrNull(getAnyValue(row, ['sacks'], null)),
    takeaways: parseNumberOrNull(getAnyValue(row, ['takeaways'], null)),
    giveAways: parseNumberOrNull(getAnyValue(row, ['giveAways', 'giveaways'], null)),
    thirdDownPercent: parseNumberOrNull(getAnyValue(row, ['thirdDownPercent'], null)),
    redZonePercent: parseNumberOrNull(getAnyValue(row, ['redZonePercent'], null)),
    ovr: parseNumberOrNull(getAnyValue(row, ['ovr'], null)),
    offenseOvr: parseNumberOrNull(getAnyValue(row, ['offenseOvr'], null)),
    defenseOvr: parseNumberOrNull(getAnyValue(row, ['defenseOvr'], null)),
    topThreats: Array.isArray(row.topThreats) ? row.topThreats : [],
    sourcePaths: [sourcePath].filter(Boolean),
  };
}

function deepHarvestFullLeagueAnalyticsRows(obj, path = '', out = [], depth = 0, seen = new Set()) {
  if (!obj || typeof obj !== 'object' || depth > 12 || seen.has(obj)) return out;
  seen.add(obj);

  if (Array.isArray(obj)) {
    if (obj.length >= 2 && obj.some(item => looksLikeFullLeagueTeamStatRow(item))) {
      obj.forEach((item, idx) => {
        if (looksLikeFullLeagueTeamStatRow(item)) {
          const normalized = normalizeFullLeagueAnalyticsRow(item, `${path}[${idx}]`);
          if (normalized) out.push({ path: `${path}[${idx}]`, raw: item, normalized });
        }
      });
    }

    obj.slice(0, 500).forEach((item, idx) => {
      deepHarvestFullLeagueAnalyticsRows(item, `${path}[${idx}]`, out, depth + 1, seen);
    });
  } else {
    if (looksLikeFullLeagueTeamStatRow(obj)) {
      const normalized = normalizeFullLeagueAnalyticsRow(obj, path);
      if (normalized) out.push({ path, raw: obj, normalized });
    }

    for (const [key, value] of Object.entries(obj)) {
      if (value && typeof value === 'object') {
        deepHarvestFullLeagueAnalyticsRows(value, path ? `${path}.${key}` : key, out, depth + 1, seen);
      }
    }
  }
...
async function harvestFullLeagueAnalyticsFromHub(context, guild, league, hub, label = 'full-league-analytics-harvester') {
  ...
  for (const item of discoveredRows) {
    mergeTeamIntoDiscoveryMap(seeded, item.normalized, item.path);
  }
  ...
  console.log('[FULL LEAGUE ANALYTICS HARVESTER 7J-5C5] ' + JSON.stringify({
    ...
    discoveredSamples: discoveredRows.slice(0, 30).map(item => ({
      path: item.path,
      team: item.normalized?.canonicalName,
      wins: item.normalized?.wins,
      losses: item.normalized?.losses,
      pf: item.normalized?.teamTotalPointsScored,
      pa: item.normalized?.teamTotalPointsAllowed,
      keys: Object.keys(item.raw || {}).slice(0, 80),
    })),
  ...
  const shouldWrite = String(process.env.EA_FULL_LEAGUE_ANALYTICS_HARVESTER_WRITE || 'true').toLowerCase() !== 'false';
  ...
  return { discoveredRows, teams, writeCandidates };
}
...
async function expandFullLeagueTeamDiscovery(context, guild, league, hub, label = 'full-discovery') {
  ...
  // Source 1: recursively discovered analytics objects from the full 811 hub.
  const recursiveObjects = deepFindMaddenTeamAnalyticsObjects(hub);
  for (const item of recursiveObjects) {
    mergeTeamIntoDiscoveryMap(discovery, item.normalized, item.path || 'hub:recursive');
  }

  // Source 2: explicit RequestInfo season-game analytics.
  ...
  if (Array.isArray(requestInfoList)) {
    for (let i = 0; i < requestInfoList.length; i++) {
      const analytics = extractRequestInfoSeasonGameAnalytics(requestInfoList[i], i);
      if (!analytics) continue;
      mergeTeamIntoDiscoveryMap(discovery, analytics.awayTeamInfo, 'careerHubInfo.requestInfoList[' + i + '].awayTeamInfo');
      mergeTeamIntoDiscoveryMap(discovery, analytics.homeTeamInfo, 'careerHubInfo.requestInfoList[' + i + '].homeTeamInfo');
    }
  }
  ...
  for (const row of scheduleRows) {
    ...
    if (hasUsableMaddenScorePair(homeScore, awayScore)) {
      mergeTeamIntoDiscoveryMap(discovery, {
        canonicalName: homeTeam,
        teamTotalPointsScored: homeScore,
        teamTotalPointsAllowed: awayScore,
      }, row.sourceLabel + ':score:home');

      mergeTeamIntoDiscoveryMap(discovery, {
        canonicalName: awayTeam,
        teamTotalPointsScored: awayScore,
        teamTotalPointsAllowed: homeScore,
      }, row.sourceLabel + ':score:away');
    }
  }

  const teams = Array.from(discovery.values()).sort((a, b) => String(a.canonicalName).localeCompare(String(b.canonicalName)));
  const pfPaTeams = teams.filter(team =>
    parseNumberOrNull(team.teamTotalPointsScored) !== null &&
    parseNumberOrNull(team.teamTotalPointsAllowed) !== null &&
    (
  ...
  const shouldWrite = String(process.env.EA_FULL_TEAM_DISCOVERY_WRITE || 'true').toLowerCase() !== 'false';
  if (shouldWrite) {
    let updated = 0;
    const updateResults = [];
...
async function synthesizeFullLeaguePfPaFromSchedule(guild, league, label = 'schedule-derived') {
  ...
  function ensure(teamName) {
    const canonical = canonicalMaddenTeamNameFromAny(teamName);
    const key = String(canonical || teamName || '').toLowerCase();
    if (!key) return null;
    if (!records.has(key)) {
      records.set(key, {
  ...
  for (const game of gamesResult.rows || []) {
    const homeTeam = canonicalMaddenTeamNameFromAny(game.home_team);
    const awayTeam = canonicalMaddenTeamNameFromAny(game.away_team);
    if (!homeTeam || !awayTeam) continue;

    const deep = resolveMaddenScoresFromRawGameDeep(game);
    const shallow = resolveMaddenScoresFromRawGame(game);

    let homeScore = parseNumberOrNull(deep.homeScore);
    let awayScore = parseNumberOrNull(deep.awayScore);
    let source = 'raw-deep';

    if (!hasUsableMaddenScorePair(homeScore, awayScore)) {
      homeScore = parseNumberOrNull(shallow.homeScore);
      awayScore = parseNumberOrNull(shallow.awayScore);
      source = 'raw-shallow';
    }

    if (!hasUsableMaddenScorePair(homeScore, awayScore)) {
      homeScore = parseNumberOrNull(game.home_score);
      awayScore = parseNumberOrNull(game.away_score);
      source = 'db-columns';
    }

    const usable = hasUsableMaddenScorePair(homeScore, awayScore);
    ...
    if (!usable) {
      ...
      if (samples.length < 20) {
        samples.push({
          ...
          usable: false,
          dbHomeScore: game.home_score,
          dbAwayScore: game.away_score,
          deep,
          shallow,
        });
      }
  ...
  const scheduleTeams = Array.from(records.values())
  ...
  const shouldWrite = String(process.env.EA_SCHEDULE_DERIVED_PFPA_WRITE || 'true').toLowerCase() !== 'false';
  if (shouldWrite && scheduleTeams.length) {
    let updated = 0;
    const updateResults = [];
  ...
  return { records, scheduleTeams, scoredGames, unscoredGames };
}
...
function summarizeEaTeamStatsProbeValue(command, requestPayload, response) {
  const value = response?.responseInfo?.value || response;
  const arrays = deepFindArraysByKey(value, [
    'teamStatInfoList',
    'teamStatsInfoList',
  ...
  return {
    ...
    commandId: command.id,
    payload: requestPayload,
    topKeys: Object.keys(value || {}).slice(0, 50),
    arrays: arrays.map(item => ({
      path: item.path,
      key: item.key,
      length: Array.isArray(item.rows) ? item.rows.length : 0,
  ...
  for (const game of gamesResult.rows) {
    ...
    if (winnerSamples.length < 16) {
      ...
      winnerSamples.push({
        ...
        scoreInfo: resolveMaddenScoresFromRawGame(game),
      });
    }
  }
...
async function runMaddenEaDirectSync(guild, league, options = {}) {
  ...
  try {
    ...
    await backfillMaddenGameScoresFromRawPayload(guild, league).catch(error => {
      console.error('[Madden Sync] Score backfill failed:', error?.message || error);
    });
...
async function exchangePersonaForMaddenToken(accessToken, persona) {
  // Snallabot selectLeague alignment:
  // 1) Call /connect/auth manually with current access token + persona info.
  // 2) Read redirect Location header.
  // 3) Extract new EA code.
  // 4) Exchange that code with token_format=JWS, client_secret, client_id.
  const personaId = persona?.personaId || persona?.persona_id || persona?.id;
  const personaNamespace = persona?.namespaceName || persona?.namespace || persona?.platform || 'xbox';

  if (!personaId) {
    throw new Error('Selected persona is missing personaId.');
  }

  if (!EA_DIRECT_CLIENT_SECRET) {
    throw new Error('EA_DIRECT_CLIENT_SECRET is missing. Add it in Railway Variables before testing select-league.');
  }
  ...
  const authUrl =
    ...
    '&persona_id=' + encodeURIComponent(String(personaId)) +
    '&persona_namespace=' + encodeURIComponent(String(personaNamespace));

  const locationUrlResponse = await fetch(authUrl, {
    redirect: 'manual',
    headers: {
  ...
  const locationUrl = locationUrlResponse.headers.get('Location') || locationUrlResponse.headers.get('location');

  if (!locationUrl) {
    throw new Error('Tried to retrieve persona-scoped EA redirect location but no Location header was returned.');
  }

  const redirectBase = EA_DIRECT_REDIRECT_URI || 'http://127.0.0.1/success';
  const locationQuery = locationUrl.includes('?') ? locationUrl.substring(locationUrl.indexOf('?')) : locationUrl.replace(redirectBase, '');
  const eaCode = new URLSearchParams(locationQuery).get('code');
  ...
  if (!tokenResponse.ok) {
    ...
    throw new Error(
      'Failed to create persona-scoped Madden access token: HTTP ' + tokenResponse.status +
      ' • ' + (tokenPayload.error_description || tokenPayload.error || tokenText).slice(0, 300) +
      ' • body=' + safeBody
    );
  }
...
function normalizeLeagueRowsFromPayload(payload) {
  if (!payload) return [];
  if (Array.isArray(payload)) return payload;
  if (Array.isArray(payload.leagues)) return payload.leagues;
  if (Array.isArray(payload.franchises)) return payload.franchises;
  if (payload.responseInfo?.value?.leagues && Array.isArray(payload.responseInfo.value.leagues)) return payload.responseInfo.value.leagues;
  if (payload.data?.leagues && Array.isArray(payload.data.leagues)) return payload.data.leagues;
  return [];
}
...
async function fetchMaddenLeaguesForPersonaToken(tokenPayload, persona) {
  ...
  if (!response.ok) {
    throw new Error('Configured Madden leagues endpoint failed: HTTP ' + response.status + ' • ' + text.slice(0, 300));
  }
...
async function completeEaTokenExchange(guildId, leagueId, userId, code) {
  ...
  if (!accessToken) {
    throw new Error('EA token exchange response did not include an access_token.');
  }
  ...
  if (EA_DIRECT_PERSONAS_URL) {
    ...
    personaCount = await saveEaPersonas(guildId, leagueId, userId, personaPayload);
    await pool.query(
      `UPDATE madden_ea_connections SET raw_personas_payload = $3, exchange_status = 'personas_fetched', updated_at = NOW()
       WHERE league_id = $1 AND user_id = $2 AND provider = 'ea_direct'`,
  ...
  const connection = await getMaddenEaConnection(leagueId, userId);
  return { connection, tokenPayload, personaCount };
}
...
function buildMaddenEaPersonaComponentsFromRows(leagueId, personas) {
  const options = personas.length
    ? personas.slice(0, 25).map(row => ({
        label: String(row.persona_name || row.persona_id).slice(0, 100),
        value: String(row.persona_id).slice(0, 100),
        description: row.platform ? String(row.platform).slice(0, 100) : 'EA Persona',
      }))
    : [{ label: 'Default EA Persona', value: 'default_persona', description: 'Temporary placeholder persona' }];
...
function buildMaddenEaFranchiseComponentsFromRows(leagueId, franchises) {
  const options = franchises.length
    ? franchises.slice(0, 25).map(row => ({
        label: String(row.franchise_name || row.franchise_id).slice(0, 100),
        value: String(row.franchise_id).slice(0, 100),
        description: row.platform ? String(row.platform).slice(0, 100) : 'Madden Franchise',
      }))
    : [{ label: 'Default Madden Franchise', value: 'default_franchise', description: 'Temporary placeholder franchise' }];
